...
function socket_handler(socket) {
  ...
  function onSocketData(data) {
    console.log('[SPIA TRACKER] Client IP:', `${remoteAddress} ; RemotePort: ${remotePort}`)
  ...
  function onSocketClose() {    
    const connection_client = `${remoteAddress}:${remotePort}`
    console.log('[SPIA TRACKER] Communication from', connection_client,
      'closed \n\tAT: ', new Date())
    /* parser_mod.files_reset() */
    ...
    if (worker_mod.conn.hasOwnProperty(connection_client)){
      ...
      console.log('[SPIA TRACKER] remove connection device:',
  ...
  function onSocketError(err) {
    console.log('[SPIA TRACKER] Error in', remoteAddress, 
      'socket:', err.message, err.stack)
    /* parser_mod.files_reset() */
  }
}
...
function socket_listener() {
  console.log('[SPIA TRACKER] Server listening on Port:', server.address().port)
}
